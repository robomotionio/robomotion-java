...
public class Spec {
	...
	public static void GenerateSpec(final String pluginName, final String version) throws ClassNotFoundException, IOException {
		...
		for (Class<?> c : classes) 
		{
			...
			if (editor != "") node.put("editor", editor);
			...
			if (inputs.size() + inputVars.size() > 0) {
                 ...
                 for (Field input : inputVars) {
                	 ...
                	 inObject.put("properties", new JObject() {{
                		 put("scope", new JObject() {{ put("type", "string"); }});
                		 put("name", new JObject() {{ put("type", "string"); }});
                	 }});
                	 
                	 String varType = ((ParameterizedType)input.getGenericType()).getActualTypeArguments()[0].toString();
                	 String[] parts = varType.split("\\.");
                	 inObject.put("variableType", parts[parts.length-1]);

                	 if (CustomScope(input)) inObject.put("customScope", true);
                	 if (MessageScope(input)) inObject.put("messageScope", true);
                	 if (MessageOnly(input)) inObject.put("messageOnly", true);
			...
			if (outputs.size() + outputVars.size() > 0) {
                ...
                for (Field output : outputVars) {
					...
					String varType = ((ParameterizedType)output.getGenericType()).getActualTypeArguments()[0].toString();
					String[] parts = varType.split("\\.");
					outObject.put("variableType", parts[parts.length-1]);
					
					if (CustomScope(output)) outObject.put("customScope", true);
					if (MessageScope(output)) outObject.put("messageScope", true);
					if (MessageOnly(output)) outObject.put("messageOnly", true);
			...
			if (options.size() + optionVars.size() > 0) {
                ...
                for (Field option : optionVars) {
					...
					String varType = ((ParameterizedType)option.getGenericType()).getActualTypeArguments()[0].toString();
					String[] parts = varType.split("\\.");
					optObject.put("variableType", parts[parts.length-1]);
					
					if (CustomScope(option)) optObject.put("customScope", true);
					if (MessageScope(option)) optObject.put("messageScope", true);
					if (MessageOnly(option)) optObject.put("messageOnly", true);
                ...
                for (Field option : options) {
	               	...
	               	if (category != ECategory.Null) optObject.put("category", category.getCategory());
	               	...
	               	if (Credential.class.isAssignableFrom(option.getType())) {
						...
						optObject.put("properties", new JObject() {{
							 put("vaultId", new JObject() {{ put("type", "string"); }});
							 put("itemId", new JObject() {{ put("type", "string"); }});
						}});
						pUISchema.put(name, new JObject() {{
	               	...
	               	if (def != null) formData.put(name, def);
                }
	...
	public static String GetVariableType(Field f) {
		Map<Class<?>, String> types = new HashMap<Class<?>, String>() {{ put(boolean.class, "Boolean"); } { put(byte.class, "Integer"); }
		 { put(short.class, "Integer"); } { put(int.class, "Integer"); } { put(long.class, "Integer"); } { put(double.class, "Double"); } 
		 { put(float.class, "Double"); } { put(Array.class, "Array"); } }; 
	...
	public static String GetType(Field f) {
		Map<Class<?>, String> types = new HashMap<Class<?>, String>() {{ put(String.class, "string"); put(boolean.class, "boolean"); } 
			{ put(byte.class, "number"); } { put(short.class, "number"); } { put(int.class, "number"); } { put(long.class, "number"); } 
			{ put(double.class, "number"); } { put(float.class, "number"); } }; 
